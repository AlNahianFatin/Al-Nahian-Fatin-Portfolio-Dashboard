...
//   education: ["institution", "degree", "field", "gpa", "startDate", "endDate", "description", "location", "imageUrl", "sortOrder", "isActive"],
...
export default function ContentEditor() {
  ...
  async function save(e: any) {
    ...
    for (const k of fs) {
      const isRequired =
        ["name", "title", "institution", "platform", "degree", "field", "status", "fileUrl"].includes(k) ||
        ...
        if (form[k] !== undefined && form[k] !== "") {
          ...
          if (isNaN(valNum) || valNum < 1) {
            ...
            const duplicate = rows.find(row =>
              row.id !== editing && row.sortOrder === valNum
            );
            if (duplicate) {
        ...
        <form onSubmit={save} className="panel p-6">
          <h2 className="font-bold">{editing ? "Edit" : "Add"} {labels(model)}</h2>
          {
            fs.map(k => {
              ...
              const isRequired = ["name", "title", "institution", "platform", "degree", "field", "status", "fileUrl"].includes(k) || (k === "url" && model === "socialLink");
                            ...
                            {val(k) && (
                              <div className="flex items-center gap-2 text-xs text-slate-500">
                                {k === 'fileUrl' ? (
                                  ...
                                  <img src={val(k) instanceof File ? URL.createObjectURL(val(k)) : val(k)} alt="Preview" className="h-10 w-10 rounded object-cover border" />
                                )}
          ...
          <div className="mt-5 flex gap-2">
            {model === "profile" ? (
              <>
                <button className="rounded-xl bg-indigo-600 px-5 py-2.5 font-semibold text-white">Update Profile</button>
              ...
              <>
                ...
                <button type="button" onClick={clearForm} className="ml-2 rounded-xl border px-5 py-2.5 font-semibold text-slate-600">Clear</button>
              </>
            )}
          </div>
              ...
              <div key={row.id} className="panel p-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex gap-3">
                    {row.imageUrl && <img src={row.imageUrl} className="h-12 w-12 rounded object-cover border" />}
                    ...
                    <div>
                      <h3 className="font-semibold">{row.name || row.title || row.institution || row.platform || row.key || row.degree || "Item"}</h3>
                      <p className="mt-1 line-clamp-2 text-sm text-slate-500">{row.email || row.description || row.url || row.value || row.degree || (row.fileUrl ? "Resume File" : "")}</p>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => edit(row)} className="rounded-lg border px-3 py-1.5 text-sm">Edit</button>
                    <button onClick={() => setDeleteId(row.id)} className="rounded-lg border border-red-200 px-3 py-1.5 text-sm text-red-600">Delete</button>
                  </div>
                </div>
              </div>
            )
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="panel max-w-sm rounded-2xl p-6 shadow-xl">
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button onClick={() => setDeleteId(null)} className="rounded-xl border px-4 py-2 text-sm font-semibold text-slate-600">Cancel</button>
              <button onClick={confirmDelete} className="rounded-xl bg-red-600 px-4 py-2 text-sm font-semibold text-white">Delete</button>
            </div>
          </div>
        </div>
      )}
